...
6. **Page limit** - hard page count for the compiled PDF. Default: **1 page** for a CV and **1 page** for a cover letter. `/apply`'s compile-and-inspect loop enforces this.
...
- **`--use default`**: remove the managed block entirely. The stock onepagecv / cover.cls guidance below it is untouched and takes over again.
...
> - `/add-template --use default` — go back to the stock <onepagecv | cover.cls> template
