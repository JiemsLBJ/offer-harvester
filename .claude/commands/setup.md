...
Build the personalized master with `cv/onepagecv.sty`. Replace the header placeholders with the user's actual name, literal phone/email text, current study status, availability, and approved photo path. Populate the default one-page sections: `个人介绍`, `教育背景`, `岗位匹配优势`, `项目经历`, `实习经历`, `获奖与荣誉`, and `技能`. Compile and visually verify one balanced A4 page; never omit factual-grounding or ATS checks to force the page count.
...
> - `cv/main_example.tex` + `cv/onepagecv.sty` - Your one-page LaTeX CV template and shared style
