...
- Extract: **company name**, **role title**, **department** (if mentioned), **location**, **application deadline** (if the posting states one), **language** of the posting (Danish or English), and any exact **email subject / resume attachment / upload filename rule**. Treat naming rules as output requirements; unresolved placeholders remain unknown and must not be guessed.
...
An existing tailored PDF is never the default output merely because it is present. If its source is legacy `moderncv`, its facts are stale, or it has not passed the current one-page render/ATS checks, regenerate it with `onepagecv` before it can be attached or uploaded.
...
- Follow the one-page single-column format from `05-cv-templates.md`, including the approved photo asset
- Tailor the profile statement and experience bullets to the specific role
- Reframe skills and achievements to match job requirements
- Keep to exactly 1 page by default; a 2-page exception requires role-critical evidence and must be reported to the user
...
- **Stock CV** uses **lualatex** with `cv/onepagecv.sty`; keep generated `.tex` files in `cv/` so the shared style and photo resolve correctly.
- Before application handoff, pass the finished CV through `apply_bot.materials`. A posting-specified filename must become the actual attachment/upload basename (`--attachment-name`, `--cv-upload-name`, or queue `resume_filename`), with the source bytes unchanged and verified.
...
- [ ] Exactly 1 page by default; any 2-page exception is justified and disclosed
- [ ] All required sections fit without clipping, overlap, or text below the bottom margin
- [ ] The photo keeps its original aspect ratio and does not disturb the header/contact line
- [ ] Dates are right-aligned and the page has balanced spacing without a large empty lower half
...
- **CV spills to page 2:** cut content using **relevance-weighted cutting** (see `05-cv-templates.md`). Remove duplicate or low-signal lines before tightening spacing; never reduce body text below 9 pt or margins below 11 mm.
- **CV is visibly sparse:** restore the highest-relevance grounded evidence or slightly increase readable spacing; do not add decorative filler.
- **Two pages are genuinely necessary:** retain only when one page would remove role-critical evidence, and disclose the exception to the user rather than silently changing the page target.
...
- [ ] **Reading order matches the visual order** — section headings appear in the same sequence as on the page, and lines from different sections are not interleaved. The stock `onepagecv` body is single-column and safe; custom templates with sidebars or multi-column layouts are where this commonly breaks.
